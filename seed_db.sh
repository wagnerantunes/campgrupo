...
# Este script injeta a configuração inicial (Seed) no Banco PostgreSQL da VPS
# Executar dentro da VPS

DB_USER="camp_user"
DB_NAME="campgrupo_assets"

# JSON Minificado do assetConfig.ts
INITIAL_JSON='{
    "logo": {
        "icon": "foundation",
        "text": "GRUPO CAMP",
...
echo "📦 Inserindo dados iniciais no Postgre..."

# Comando SQL para inserir ou atualizar
SQL_CMD="INSERT INTO site_config (key, data) VALUES ('current_config', '$INITIAL_JSON') ON CONFLICT (key) DO UPDATE SET data = '$INITIAL_JSON';"

# Executar como usuario postgres, trocando para o nosso user/db
